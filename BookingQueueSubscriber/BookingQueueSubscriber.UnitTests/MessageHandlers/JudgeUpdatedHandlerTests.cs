...
namespace BookingQueueSubscriber.UnitTests.MessageHandlers
{
    public class JudgeUpdatedHandlerTests : MessageHandlerTestBase
    {
        ...
        {
            var messageHandler = new JudgeUpdatedHandler(VideoApiServiceMock.Object, NotificationServiceMock.Object, _logger.Object);
            ...
            NotificationServiceMock.Verify(x => x.SendNewSingleDayHearingConfirmationNotification(It.IsAny<HearingDto>(), It.IsAny<ParticipantDto[]>()), Times.Once);
        }
        [Test]
        public async Task should_not_send_notification_but_still_update_participant_details()
        {
            ...
            await messageHandler.HandleAsync(integrationEvent);
            VideoApiServiceMock.Verify(x => x.UpdateParticipantDetails(It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<UpdateParticipantRequest>()), Times.Once);
            NotificationServiceMock.Verify(x => x.SendNewSingleDayHearingConfirmationNotification(It.IsAny<HearingDto>(), It.IsAny<ParticipantDto[]>()), Times.Never);
        }
    }
}